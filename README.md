# Details for the Notes App

## Reasons

The notes app uses the `create react app` and `Boostrap`.

The reason to choose both was to explore those 2 frameworks. I have read a lot about them, but never really got to do a fully functional app with them.

The `create react app` uses `babel` to transpiler the es6 code so it can be used in older browsers.
...
Unfortunately, I haven't had the time to write any tests yet so the command `npm tests` will do nothing.

## Further changes

With more time I will probably split a bit more the NotesApp creating another component that does the left menu render.
...
* `moment` : To deal with date and time, specifically in the formatting
* `react-router-dom` : To deal with the routes
...
To follow is the original details from the create react app README.md file.
